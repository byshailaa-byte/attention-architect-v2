...
export default async function ReportPage({
  ...
  // Simplified sessions use the Phase B report; redirect if they land here directly.
  if (row.pricing_variant === "simplified") {
    redirect(`/preview/simplified-v1?session=${sessionId}`);
  }
  ...
  if (isGated) {
    ...
    // Control arm: check for a paid purchase so the report can hide PriceCards
    const paidRows = (await sql`
