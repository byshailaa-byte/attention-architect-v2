...
// Fixed closing sentence appended to m_teaser in code — never generated by the model,
// never varied per family. Exported so auto-generate can re-append it if the quality
// engine regenerates m_teaser (regeneration via generateMoment() returns bare prose).
export const TEASER_FIXED_CLOSE = "Patterns like this tend to get more workable the earlier they're named clearly — that's just how habits settle, for kids and adults alike.";
...
export async function composeReport(ctx: NarrativeContext): Promise<ComposeOutput> {
  ...
  // or sessions created before Phase 23 migration).
  {
    ...
    // TEASER_FIXED_CLOSE is defined at module scope (above composeReport) and exported.
