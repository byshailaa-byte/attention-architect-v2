...
export async function composeReport(ctx: NarrativeContext): Promise<ComposeOutput> {
  ...
  // or sessions created before Phase 23 migration).
  {
    ...
    // (1) the parent's worry label — for the opening reflection only,
    // (2) the parent's follow-up answer — for the opening reflection only,
    // (3) G1/G2 evidence nodes — REQUIRED source for the bridge sentence.
    // The bridge sentence must add something the follow-up answer did not already say.
    ...
    if (ctx.worryFollowup && ctx.concerns.length > 0) {
      const worryLabel = CONCERN_CARD_LABELS[ctx.concerns[0]] ?? "what you told us";
      teaserSpec = track({
        momentId: "m_teaser",
        momentType: "recognition",
        section: "Teaser",
        ...
        evidenceText: [
          ...
          `Parent's follow-up answer — for the OPENING REFLECTION only, not the bridge: ${ctx.worryFollowup}`,
          ...evidenceLines(ctx, ["G1", "G2"]).map(e => `[REQUIRED for bridge sentence — this is where the new, unstated detail must come from]: ${e}`),
        ],
        additionalInstruction:
          `EVIDENCE INPUTS:\n` +
