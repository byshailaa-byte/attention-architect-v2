...
export async function composeReport(ctx: NarrativeContext): Promise<ComposeOutput> {
  ...
  // ── 0. Cover + 1. Recognition (+ optional m_teaser) ─────────────────────────
  // All three run in parallel — none uses priorContext.
  // m_teaser: pre-payment recognition teaser for the gated arm. Generated here so it
  // shares the same no-prior-context window as Cover and Recognition.
  // Skipped when worry_followup is absent (sessions that bypassed the landing page flow,
  // or sessions created before Phase 23 migration).
  {
    ...
    // m_teaser: pre-payment recognition teaser. Evidence scope is intentionally narrow:
    // (1) the parent's worry label + follow-up echo text (primary, required anchor),
    // (2) the same G1+G2 nodes as m_01 (optional, secondary only).
    // The fixed closing sentence is appended in code — never generated by the model.
    ...
    let teaserSpec: MomentSpec | null = null;
    if (ctx.worryFollowup && ctx.concerns.length > 0) {
      const worryLabel = CONCERN_CARD_LABELS[ctx.concerns[0]] ?? ctx.concerns[0];
      teaserSpec = track({
        momentId: "m_teaser",
        momentType: "recognition",
        section: "Teaser",
        purpose: "pre-payment recognition teaser — reflect the parent's specific worry and one child-specific detail from their follow-up answer; recognition only, no explanation",
        emotionalObjective: "precise recognition without resolution",
        confidenceTier: bestTierFor(ctx, ["attention_shape", "attention_competition"]),
        behaviourNodeRefs: bgNodeIdsForDims(ctx, ["attention_shape", "attention_competition"]),
        humanDecisionRefs: hdgNodesForQuestions(ctx, ["G1", "G2"]).map(n => n.id),
        evidenceText: [
          `Parent's stated worry (use this natural phrasing, not the raw key): ${worryLabel}`,
          `Parent's follow-up answer — REQUIRED anchor for the bridge sentence: ${ctx.worryFollowup}`,
          ...evidenceLines(ctx, ["G1", "G2"]).map(e => `[Optional context, secondary only — do not use if follow-up alone is specific enough]: ${e}`),
        ],
        additionalInstruction:
          `EVIDENCE SCOPE: Two sources only — (1) the parent's stated worry and follow-up answer above, ` +
          `(2) the scoped G1/G2 evidence nodes marked as optional. ` +
          `Do not draw from anything else in the FAMILY CONTEXT block.\n\n` +
          `STRUCTURE:\n` +
          `1. Open by reflecting "${worryLabel}" back in your own plain words. ` +
          `Do NOT use the construction "you came here worried about X" or "you've watched this enough that X is the word you brought here" ` +
          `— that exact sentence shape appears later in the report; find a different way to say it.\n` +
          `2. Bridge to ONE specific detail, anchored primarily in the follow-up answer ` +
          `("${ctx.worryFollowup}") — this is what actually happens, in the parent's own words. ` +
          `This sentence must feel unmistakably specific to this child: if it could be copied unchanged into another family's teaser, it is too generic.\n` +
    ...
    const [coverMoment, m01Moment, teaserResult] = await Promise.all([
      generateMoment(coverSpec, ctx),
      generateMoment(m01Spec, ctx),
      teaserSpec ? generateMoment(teaserSpec, ctx) : Promise.resolve(null as AttentionMoment | null),
    ]);
    moments.push(coverMoment, m01Moment);
    if (teaserResult) {
      moments.push({ ...teaserResult, content: `${teaserResult.content}\n\n${TEASER_FIXED_CLOSE}` });
    }
  }
